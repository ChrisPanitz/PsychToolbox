...
frPerCyc = [20 14]; % duration of one cycle in frames (for different frequencies)
flickMods = {'box'}; % how flicker stimulus is modulated. 'box' or 'sin' or both possible
frPerMetaCyc = 140; % meta cycle is the time window after which all frequencies are in phase again (here in frames)
metaCyclesTotal = 5; % trial length in meta cycles
metaCyclesBaseline = 1:2; % how many metacycle windows can the baseline last
...
try
    ...
    % driving frequency of first Array (depends on cueLetters and frPerCyc)
    % here, 1: Color = 6 Hz, 2: Color = 8.57 Hz
    ...
    % here, 1 = attended = 6 Hz, 2 = attended = 8.57 Hz
    ...
    for trialI = 1:length(cueEventFrames)
        if nrOfEventsVecColor(trialI)+nrOfEventsVecGray(trialI) > 0
            ...
            while eventsValid == false
                ...
                eventLats = randi([blLengthVec(trialI)+1, durInFrames-cueEventDur], nrOfEventsVecColor(trialI), 1);
                for latI = 1:length(eventLats)
